...
def _draw_depth_layer(width: int, height: int, z: int,
    ...
    # The DAW is a floating instrument bay inside the desktop instead of a
    # maximized application surface. The bay is deliberately larger than the
    # REAPER window so its stepped cyan/magenta rails remain visible around the
    # client window and visually seat it in the 24-plane workspace.
    bay_x0, bay_y0 = DOCK_W + 126 + offset, TOP + 62 + offset
    bay_x1, bay_y1 = width - 126 + offset, height - 92 + offset
    draw.rounded_rectangle(
        [bay_x0, bay_y0, bay_x1, bay_y1],
        radius=18,
        fill=(7, 13, 24, min(28, shadow_alpha)),
        outline=(114, 239, 255, min(58, edge_alpha + 12)),
        width=1,
    )
    draw.line(
        [(bay_x0 + 22, bay_y0), (bay_x1 - 22, bay_y0)],
        fill=(255, 106, 203, min(70, edge_alpha + 20)),
        width=2,
    )
    draw.line(
        [(bay_x0, bay_y1 - 22), (bay_x1, bay_y1 - 22)],
        fill=(255, 179, 107, min(42, edge_alpha + 8)),
        width=1,
    )
    draw.line(
        [(bay_x0, bay_y0 + 24), (bay_x0, bay_y1 - 24)],
        fill=(114, 239, 255, min(64, edge_alpha + 16)),
        width=2,
    )
    draw.line(
        [(bay_x1, bay_y0 + 24), (bay_x1, bay_y1 - 24)],
        fill=(255, 106, 203, min(48, edge_alpha + 8)),
        width=1,
    )
    ...
    if z == 24:
        ...
        draw.text(
            (DOCK_W + 142, TOP + 48),
            "DAW BAY  /  ASTRA FLOATING MIX SURFACE",
            font=_font(12, True),
            fill=(114, 239, 255, 150),
        )
